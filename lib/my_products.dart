...
class my_pro extends StatefulWidget {
  const my_pro({Key key}) : super(key: key);
  static const routNamed = "my_pro";

  @override
  _my_proState createState() => _my_proState();
}

class _my_proState extends State<my_pro> {
  List<Product> pro = [];
  ...
  void initState() {
    ...
    proRef.once().then((DataSnapshot snap) {
      ...
      for (var individualKey in KEYS) {
        ...
        pro.add(product);}else print ("not yours");
        ...
        body: pro.length == 0
          ...
          itemBuilder: (_, index) {
            return FlatButton(
              ...
              child: Column(
                children: [//if(pro[index].uid==FirebaseAuth.instance.currentUser.uid)
